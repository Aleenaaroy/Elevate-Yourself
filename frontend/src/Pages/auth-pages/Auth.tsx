...
import { useOtpHandler } from "../../utils/otpHandler";
...
const Auth: React.FC<AuthProps> = ({ isLogin }) => {
  const { userData, setIsLoading, handleChangeData, isLoading } =
    useAuthFormHook();
  const { showOtpModal } = useOtpHandler();
                ...
                <>
                  {isLogin ? (
                    <p className='mt-10 text-center text-sm text-gray-500"'>
                      ...
                      <Link
                        to={"/register"}
                        className="font-semibold leading-6 text-violet-900 hover:text-indigo-500"
                      >
                        Register
                      </Link>
                    </p>
            ...
            <div className="w-full md:w-1/2 p-8">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {isLoading ? (
                  ...
                  <button
                    type="submit"
                    className="w-full bg-indigo-600 text-white font-semibold px-4 py-2 rounded-md hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                    {isLogin ? "Login" : "Signup"}
                  </button>
                )}
                <p className="lg:text-lg font-semibold text-center py-1 text-black">
                  OR
                </p>
                  ...
                  <GoogleLogin
                    onSuccess={(credRes) => {
                      if (!isLogin) {
                        ...
                        axiosInstance
                          .post("/auth/google/login", credRes)
                          .then((res) => {
                            if (res.data.message) {
                          ...
                          .catch((err) =>
                            console.log(err, "axio catch err g login")
                          );
                      }
                    }}
                    onError={() => {
                      errorPopAlert(
                        "Google authentication failed! Please Try Later"
                      );
                      return;
                    }}
                    type="standard"
